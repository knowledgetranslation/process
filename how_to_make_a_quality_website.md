...
* [6:13 Use Frames when Functions Must Remain Accessible](http://guidelines.usability.gov/guidelines/66)
...
1. Every website needs to be [responsive](https://en.wikipedia.org/wiki/Responsive_web_design)[^1], every website needs to be [accessible](https://en.wikipedia.org/wiki/Web_accessibility)[^2],[^3], every website needs to use [progressive enhancement](https://en.wikipedia.org/wiki/Progressive_Enhancement)[^4].

1. Every stakeholder is a team member and every team member is a stakeholder.[^5]
	...
	* Exception: Usability tests should, ideally, be conducted and summarized by people completely separate from the rest of the team, to avoid bias.
		...
		If the team members are the ones conducting usability tests, every effort should be made to avoid bias in testing.[^6]
...
1. Don’t give disproportionate or inappropriate weight to trivial issues.[^7]
...
1. Remember the [Priority of Constituencies](http://www.w3.org/TR/html-design-principles/#priority-of-constituencies), a founding web design principle encoded by the World Wide Web Consortium and Web Hypertext Application Technology Working Group (the organizations responsible for web standards):
	...
	* Note that this principle does not mean we should always defer to what each constituency wants or says they want; it means that in cases of conflict we should look at evidence and use that evidence to prioritize benefits to users over authors, etc.[^8] We always prioritize evidence over expert opinion.

1. No single user is more important than another user.[^9] 
...
1. Usability tests are for identifying problems, not for generating suggestions.
	...
	* “Users are most valuable in helping designers know what a system should do, but not in helping designers determine how best to have the system do it.”[^10]
...
1. Avoid requiring users to log in with a username and password.
	...
	* If a password cannot be avoided, show it in plain text with an option to obscure it[^12] and provide an automated way for users to reset their own passwords.
...
1. Publish what you publish. Knowledge translation depends on dissemination; we need share what we’re creating[^13].
...
1. Using your operating budget and timelines as guides, determine the ideal project scope[^14], then reduce it by half. This is your scope for *this phase*.
...
1. Research
	...
	* Study the data collected from other projects and earlier versions of this project, and interview stakeholders (especially users). Determine what content and features are important to whom and why, how they will be used, and where they will be accessed from.[^10],[^15]
...
1. Develop or choose an existing style guide for content, *before* you actually start creating any of the content.[^18]
...
1. Information Architecture: create a plan for how your content will be structured, both at a high-level (e.g. site navigation), and a low-level (e.g. sections within documents/pages). Then, reduce the complexity of your Information Architecture by half.[^19],[^20],[^21],[^22]

	* Content should be structured as simply as possible to help users find what they’re looking for.[^21],[^22]
	
1. Create your content, in compliance with your style guide and Information Architecture, and keeping in mind your user personas. Then, delete half the content.[^23]

	* Your users usually have very specific goals when visiting a web site. Don’t clutter their experience, and don’t give them what they don’t need.[^15],[^24],[^23]

1. Conduct usability testing on the content. Can users understand it and get the info they need?[^10]
...
1. Create basic, works-for-everybody design templates that account for all the content and work well for all users, regardless of:
	...
	* Javascript and plugin support/availability[^28]
	* Language, e.g. with machine-translation support
	
	This isn’t the “showcase” design, this is the fallback, lowest-common-denominator design.[^9]
	
1. Cut the number of templates in half, and for each remaining template, cut the number of user interace elements in half[^29]. Double the font size, and double the white space.[^30],[^31]

1. Conduct usability testing on this design, using paper prototypes.[^9],[^15],[^32]
...
1. Design “showcase” design templates for how everything should look when printed in the best possible case: modern device, modern browser, available JavaScript.[^25],[^26],[^27]
...
1. Perform Search Engine Optimization by:
	...
	* Ensuring proper meta-information and titles have been used for all pages, including the use of structured data (e.g. [schema.org](http://schema.org/)).[^33]
...
1. Test the templates in as many browsers as possible[^16], on as many devices as possible, with as many assistive technologies as possible. Make sure everything functions as it should and looks ok (though not necessarily pixel perfect!).
...
1. Conduct automated markup and accessibility[^3] validation.

1. Fix any problems that arose during validity testing.

	* If there were problems to fix, return to step 1.

1. Conduct automated performance testing[^36],[^37],[^38].

1. Fix any problems that arose during performance testing.

	* Good performance should take priority over good aesthetics. Ideally you can achieve both, but in cases of conflict, optimize for performance.[^39]
