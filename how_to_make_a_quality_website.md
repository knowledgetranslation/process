...
* [6:13 Use Frames when Functions Must Remain Accessible](http://guidelines.usability.gov/guidelines/66)[^1a]
...
1. Every website needs to be [responsive](https://en.wikipedia.org/wiki/Responsive_web_design)[^2a],[^3a], every website needs to be [accessible](https://en.wikipedia.org/wiki/Web_accessibility)[^4a],[^5a], every website needs to use [progressive enhancement](https://en.wikipedia.org/wiki/Progressive_Enhancement)[^6a],[^7a].

1. Every stakeholder is a team member and every team member is a stakeholder.[^8a]
	...
	* Exception: Usability tests should, ideally, be conducted and summarized by people completely separate from the rest of the team, to avoid bias.
		...
		If the team members are the ones conducting usability tests, every effort should be made to avoid bias in testing.[^9a]
...
1. Involve all stakeholders at every step of the project, every meeting, and every decision point. Everybody should have the *opportunity* to participate.
	...
	* e.g. designers should be involved during content creation, developers should be involved while creating timelines, users should be involved while planning scope[^10a], etc.
...
1. Don’t give disproportionate or inappropriate weight to trivial issues.[^11a]
...
1. Remember the [Priority of Constituencies](http://www.w3.org/TR/html-design-principles/#priority-of-constituencies), a founding web design principle encoded by the World Wide Web Consortium and Web Hypertext Application Technology Working Group (the organizations responsible for web standards):
	...
	* Note that this principle does not mean we should always defer to what each constituency wants or says they want; it means that in cases of conflict we should look at evidence and use that evidence to prioritize benefits to users over authors, etc.[^12a] We always prioritize evidence over expert opinion.

1. No single user is more important than another user.[^13a] 
...
1. Usability tests are for identifying problems, not for generating suggestions.
	...
	* “Users are most valuable in helping designers know what a system should do, but not in helping designers determine how best to have the system do it.”[^14a]
...
1. Usability testing should only ever focus on one area at a time, e.g. either content, look-and-feel, or functionality. If you want to test multiple areas, conduct separate usability tests.
	...
	* Usability tests should have specific performance and success goals.[^15a]
...
1. Avoid requiring users to log in with a username and password.
	...
	* If a password cannot be avoided, show it in plain text with an option to obscure it[^16a] and provide an automated way for users to reset their own passwords.
...
1. Publish what you publish. Knowledge translation depends on dissemination; we need share what we’re creating[^17a].
...
1. Using your operating budget and timelines as guides, determine the ideal project scope[^18a], then reduce it by half. This is your scope for *this phase*.
...
1. Develop or choose an existing style guide for content, *before* you actually start creating any of the content.[^22a]
...
1. Information Architecture: create a plan for how your content will be structured, both at a high-level (e.g. site navigation), and a low-level (e.g. sections within documents/pages). Then, reduce the complexity of your Information Architecture by half.[^23a],[^24a],[^25a],[^26a]

	* Content should be structured as simply as possible to help users find what they’re looking for.[^25a],[^26a]
	
1. Create your content, in compliance with your style guide and Information Architecture, and keeping in mind your user personas. Then, delete half the content.[^27a]

	* Your users usually have very specific goals when visiting a web site. Don’t clutter their experience, and don’t give them what they don’t need.[^19a],[^28a],[^27a],[^29a]

1. Conduct usability testing on the content. Can users understand it and get the info they need?[^14a]

	* Try to test with *real* users, and/or users that closely match each of your personas.[^10a]
...
1. Create basic, works-for-everybody design templates that account for all the content and work well for all users, regardless of:
	...
	* Printed vs. on-screen[^30a],[^31a],[^32a]
	* Device, e.g. phone, tablet, desktop, laptop, etc.[^7a]
	...
	* Javascript and plugin support/availability[^33a],[^34a]
	...
	* Script: No support for client side scripting.
	...
	This isn’t the “showcase” design, this is the fallback, lowest-common-denominator design.[^13a]
	
1. Cut the number of templates in half, and for each remaining template, cut the number of user interace elements in half[^35a]. Double the font size, and double the white space.[^36a],[^37a]

1. Conduct usability testing on this design, using paper prototypes.[^13a],[^19a],[^38a]
...
1. Create “showcase” design templates for how everything should look in a mobile browser in the best possible case: modern device, modern browser, fast connection, available JavaScript.[^39a]
...
1. Design “showcase” design templates for how everything should look when printed in the best possible case: modern device, modern browser, available JavaScript.[^30a],[^31a],[^32a]
...
1. Perform Search Engine Optimization by:
	...
	* Ensuring proper meta-information and titles have been used for all pages, including the use of structured data (e.g. [schema.org](http://schema.org/)).[^40a]
...
1. Create a “help” section.[^41a],[^42a]
...
1. Test the templates in as many browsers as possible[^20a], on as many devices as possible, with as many assistive technologies as possible. Make sure everything functions as it should and looks ok (though not necessarily pixel perfect!).

	* Prioritize testing with real browsers and devices, and only use emulators/simulators if the real thing is unavailable.[^43a]
